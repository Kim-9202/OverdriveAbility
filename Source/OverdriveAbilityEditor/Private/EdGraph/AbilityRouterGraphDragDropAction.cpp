...
FAbilityRouterGraphDragDropAction::FAbilityRouterGraphDragDropAction(const TSharedRef<SGraphPanel>& InGraphPanel, const TArray<FGraphPinHandle>& DraggedPins)
...
TSharedRef<FAbilityRouterGraphDragDropAction> FAbilityRouterGraphDragDropAction::New(const TSharedRef<SGraphPanel>& InGraphPanel, const TArray<FGraphPinHandle>& InGraphPinHandles)
{
	TSharedRef<FAbilityRouterGraphDragDropAction> NewOperation = MakeShareable<FAbilityRouterGraphDragDropAction>(new FAbilityRouterGraphDragDropAction(InGraphPanel, InGraphPinHandles));
	NewOperation->Construct();

	return NewOperation;
}

void FAbilityRouterGraphDragDropAction::OnDrop(bool bDropWasHandled, const FPointerEvent& MouseEvent)
{
...
void FAbilityRouterGraphDragDropAction::HoverTargetChanged()
{
	...
	else if (UEdAbilityRouterGraphNode* TargetNodeObj = Cast<UEdAbilityRouterGraphNode>(GetHoveredNode()))
	{
		TArray<UEdGraphPin*> ValidSourcePins;
		ValidateGraphPinList(/*out*/ ValidSourcePins);
...
FReply FAbilityRouterGraphDragDropAction::DroppedOnPin(FVector2D ScreenPosition, FVector2D GraphPosition)
{
	TArray<UEdGraphPin*> ValidSourcePins;
	ValidateGraphPinList(/*out*/ ValidSourcePins);
...
FReply FAbilityRouterGraphDragDropAction::DroppedOnNode(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
FReply FAbilityRouterGraphDragDropAction::DroppedOnPanel(const TSharedRef<SWidget>& Panel, FVector2D ScreenPosition, FVector2D GraphPosition, UEdGraph& Graph)
{
	// Gather any source drag pins
...
void FAbilityRouterGraphDragDropAction::OnDragged(const FDragDropEvent& DragDropEvent)
{
...
void FAbilityRouterGraphDragDropAction::ValidateGraphPinList(TArray<UEdGraphPin*>& OutValidPins)
{
